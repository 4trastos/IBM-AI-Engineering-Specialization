...
def build_model(state_size, action_size):
    ...
    model.add(action_size, activation='lineal')
...
def remeber(state, action, reward, next_state, done):
...
def replay(bacth_size):
    global epsilon
    minibatch = random.sample(memory, bacth_size)
...
for e in range(episodes):
    ...
    for time in range(200):
        ...
        remeber(state, action, reward, next_state, done)
...
# Evaluatin loop performance agent
